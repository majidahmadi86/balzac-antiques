// ---------------------------------------------------------------------------
// UI translations — EN authored, FR drafted by the studio and PENDING CLIENT
// REVIEW before this can be called final (client sells to French speakers).
// Product/category DATA stays EN until the admin panel provides per-language
// fields (contract scope). Keys are flat "section.name".
// ---------------------------------------------------------------------------

export type Locale = "en" | "fr";

export const dict: Record<Locale, Record<string, string>> = {
  en: {
    "nav.collection": "Collection",
    "nav.theCollection": "The Collection",
    "nav.viewAll": "View All",
    "nav.allPieces": "All Pieces",
    "nav.acquisitions": "Acquisitions",
    "nav.about": "About",
    "nav.contact": "Contact",
    "nav.menu": "Menu",
    "nav.language": "Language",
    "nav.currency": "Currency",
    "hero.title": "Discover Remarkable Objects",
    "hero.subtitle": "Books, Art, Watches, Music & More",
    "hero.cta": "Browse All",
    "hero.viewPiece": "View Piece",
    "feat.eyebrow": "Featured",
    "feat.title": "Exceptional Pieces",
    "auction.viewLots": "View Lots",
    "story.quote":
      "\u201CEvery object in our collection carries a history worth continuing.\u201D",
    "story.body":
      "Balzac Antiques sources, authenticates, and presents rare books, fine watches, and remarkable objects for collectors who value craftsmanship and provenance as much as the piece itself.",
    "badge.ship1": "Worldwide",
    "badge.ship2": "Shipping",
    "badge.auth1": "Authenticity",
    "badge.auth2": "Guaranteed",
    "badge.advice1": "Expert Advice",
    "badge.advice2": "Personal Service",
    "footer.blurb":
      "Curated rare books, fine watches, and remarkable objects, sourced and authenticated in Bangkok for collectors worldwide.",
    "footer.collections": "Collections",
    "footer.balzac": "Balzac",
    "footer.aboutUs": "About Us",
    "footer.stayInformed": "Stay Informed",
    "footer.brandBlurb": "Curated rare books, fine watches, and remarkable objects, sourced and authenticated in Bangkok for collectors worldwide.",
    "cat.books": "Books",
    "cat.art": "Art",
    "cat.watches": "Watches",
    "cat.music": "Music",
    "cat.furniture": "Furniture",
    "cat.fashion": "Fashion",
    "cat.properties": "Properties",
    "cat.curiosities": "Curiosities",
    "footer.newsBlurb": "New acquisitions and upcoming auctions, occasionally.",
    "footer.email": "Email address",
    "footer.join": "Join",
    "footer.newsThanks": "Thank you. You are on the list.",
    "footer.newsErr": "Please enter a valid email address.",
    "footer.rights": "All rights reserved.",
    "legal.terms": "Terms of Sale",
    "legal.privacy": "Privacy",
    "legal.shipping": "Shipping",
    "legal.returns": "Returns",
    "about.eyebrow": "About Us",
    "about.title": "Rare Objects. Timeless Stories.",
    "about.lead": "Balzac Antiques is a curated destination for rare and timeless objects.",
    "about.p1":
      "Our collection includes rare books, vintage vinyl records, fine watches, artworks, iconic design pieces, decorative objects, and carefully selected collectibles. Every item is chosen for its authenticity, quality, rarity, and lasting appeal.",
    "about.p2":
      "We believe that great objects carry history, craftsmanship, and character. Whether you are an experienced collector or simply looking for something unique, we are committed to offering carefully sourced pieces that deserve to be appreciated for generations to come.",
    "about.p3":
      "Sourcing across Europe and Asia, Balzac Antiques is built on a passion for quality, authenticity, and thoughtful curation.",
    "about.tagline": "Balzac Antiques \u00b7 Rare Objects. Timeless Stories.",
    "about.browse": "Browse the Collection",
    "about.contact": "Contact Us",
    "contact.eyebrow": "Contact",
    "contact.title": "Contact Us",
    "contact.lead":
      "We would be delighted to assist you with any enquiries regarding our collection, sourcing requests, or private acquisitions.",
    "contact.email": "Email",
    "contact.ch": "Switzerland",
    "contact.asia": "Asia",
    "contact.forward": "We look forward to hearing from you.",
    "contact.formTitle": "Send a Message",
    "contact.name": "Your name",
    "contact.msg": "Message",
    "contact.send": "Send Message",
    "contact.sent": "Thank you. Your message has been received and the gallery will reply to you by email.",
    "contact.err": "Please check your details and try again.",
    "sell.eyebrow": "Sell With Us",
    "sell.title": "Acquisitions",
    "sell.lead":
      "Balzac Antiques is actively seeking rare books, works of art, vintage watches, vinyl records, iconic design, decorative arts, and quality collectibles.",
    "sell.i1": "Rare books",
    "sell.i2": "Works of art",
    "sell.i3": "Vintage watches",
    "sell.i4": "Vinyl records",
    "sell.i5": "Iconic design",
    "sell.i6": "Decorative arts",
    "sell.i7": "Quality collectibles",
    "sell.p1":
      "If you wish to offer an item or an entire collection, please send us photographs and a brief description.",
    "sell.p2": "All enquiries are handled in complete confidence.",
    "sell.cta": "Submit an Item",
    "sell.orWrite": "Or write to us directly at",
    "sell.formTitle": "Tell Us About Your Piece",
    "sell.details": "About the piece",
    "sell.detailsHint": "Item or collection, approximate period, condition, provenance if known, and any asking price.",
    "sell.send": "Send Details",
    "sell.sent": "Thank you. Your submission has been received and the gallery will be in touch by email.",
    "sell.photos": "Photographs (optional)",
    "sell.photosHint": "Up to 3 images, JPG or PNG. Clear daylight photographs help us assess a piece.",
    "sell.confidenceNote": "All enquiries are handled in complete confidence.",
    "coll.eyebrow": "The Collection",
    "coll.title": "All Pieces",
    "coll.lead":
      "Every item is chosen for its authenticity, quality, rarity, and lasting appeal.",
    "coll.all": "All",
    "coll.emptyTitle": "No pieces currently listed in {c}.",
    "coll.emptyBody":
      "New acquisitions are added regularly. If you are looking for something particular, we are happy to source it for you.",
    "coll.enquiry": "Make an Enquiry",
    "coll.viewAllPieces": "View All Pieces",
    "prod.backTo": "Back to",
    "prod.condition": "Condition",
    "prod.dimensions": "Dimensions",
    "prod.enquire": "Enquire About This Piece",
    "prod.shipping": "Worldwide shipping available",
    "prod.sold": "Sold",
    "prod.soldNote": "This piece has found a new home.",
    "prod.reserved": "Reserved",
    "prod.reservedNote": "This piece is currently on hold.",
    "drawer.tagline": "Rare Objects. Timeless Stories.",
    "nav.account": "Account",
    "auth.loginTitle": "Sign In",
    "auth.loginSubtitle": "Access your account and your orders.",
    "auth.loginError": "Incorrect email or password. Please try again.",
    "auth.email": "Email",
    "auth.password": "Password",
    "auth.signInCta": "Sign In",
    "auth.noAccount": "New to Balzac Antiques?",
    "auth.createOne": "Create an account",
    "auth.registerTitle": "Create Account",
    "auth.registerSubtitle": "Save your details and follow your acquisitions.",
    "auth.name": "Full Name",
    "auth.passwordHint": "At least 8 characters.",
    "auth.createCta": "Create Account",
    "auth.haveAccount": "Already have an account?",
    "auth.signInLink": "Sign in",
    "auth.errName": "Please enter your name.",
    "auth.errEmail": "Please enter a valid email address.",
    "auth.errPassword": "Password must be at least 8 characters.",
    "auth.errTaken": "An account with this email already exists.",
    "account.welcome": "Welcome back",
    "account.signOut": "Sign Out",
    "account.ordersTitle": "Orders",
    "account.ordersEmpty": "You have no orders yet.",
    "account.ordersNote": "Your acquisitions will appear here.",
    "account.browse": "Browse the Collection",
    "account.addressesTitle": "Saved Addresses",
    "account.addressesEmpty": "No saved addresses yet.",
    "account.addressesNote": "You can add one during checkout.",
    "addr.title": "Addresses",
    "addr.backToAccount": "Back to account",
    "addr.backToAddresses": "Back to addresses",
    "addr.add": "Add an address",
    "addr.manage": "Manage addresses",
    "addr.onFile": "Your saved delivery addresses.",
    "addr.none": "You have no saved addresses.",
    "addr.line1": "Address",
    "addr.line2": "Apartment, suite, etc. (optional)",
    "addr.city": "City",
    "addr.postcode": "Postal code",
    "addr.country": "Country",
    "addr.countryPlaceholder": "Select a country",
    "addr.default": "Default",
    "addr.setDefault": "Set as default",
    "addr.makeDefault": "Use as my default address",
    "addr.save": "Save address",
    "addr.saveChanges": "Save changes",
    "addr.edit": "Edit",
    "addr.delete": "Delete",
    "addr.err": "Please complete all required fields.",
    "addr.saved": "Address saved.",
    "addr.deleted": "Address deleted.",
    "addr.editTitle": "Edit address",
    "nav.cart": "Cart",
    "cart.title": "Your Cart",
    "cart.empty": "Your cart is empty.",
    "cart.browse": "Browse the Collection",
    "cart.add": "Add to Cart",
    "cart.inCart": "In your cart",
    "cart.view": "View cart",
    "cart.remove": "Remove",
    "cart.subtotal": "Subtotal",
    "cart.shippingNote": "Shipping calculated at checkout.",
    "cart.checkout": "Proceed to Checkout",
    "cart.continue": "Continue shopping",
    "cart.close": "Close",
    "checkout.title": "Checkout",
    "checkout.yourDetails": "Your Details",
    "checkout.phone": "Phone (optional)",
    "checkout.shippingAddress": "Shipping Address",
    "checkout.useDifferent": "Use a different address",
    "checkout.passwordTitle": "Create a Password (optional)",
    "checkout.passwordHint": "Optional. Lets you sign in later to follow your order.",
    "checkout.summary": "Your Order",
    "checkout.reserveNote": "Placing this order reserves the piece for you. The gallery will contact you by email to arrange payment and delivery.",
    "checkout.placeOrder": "Place Order",
    "checkout.errUnavailable": "We are sorry, this piece has just been taken by another collector:",
    "checkout.removeAndContinue": "Remove it and continue",
    "checkout.errEmail": "An account with this email already exists.",
    "checkout.payNote": "Placing this order reserves the piece for you and takes you to our secure payment page.",
    "checkout.continuePayment": "Continue to Payment",
    "pay.testMode": "Test mode. No real payment is taken and no money changes hands. Use card number 4242 4242 4242 4242, any future expiry date, any CVC and any postcode.",
    "conf.paidEyebrow": "Payment received",
    "conf.paidTitle": "Thank You, Your Payment Has Been Received",
    "conf.paidNext": "The piece is yours. The gallery will be in touch by email to arrange delivery.",
    "conf.awaitTitle": "Your Order Is Reserved",
    "conf.awaitNext": "The piece is held for you and your order is not complete until payment is made. You can pay securely below.",
    "conf.payNow": "Pay Now",
    "conf.payError": "The payment page could not be opened just now. Please try again, or contact the gallery and we will send you a payment link.",
    "conf.eyebrow": "Thank you",
    "conf.title": "Your Order Has Been Received",
    "conf.next": "Your piece is now reserved for you. The gallery will contact you by email shortly to arrange payment and delivery.",
    "conf.number": "Order reference:",
    "conf.items": "Pieces",
    "conf.shipTo": "Delivery Address",
    "conf.continue": "Continue browsing the Collection",
    "orders.title": "My Orders",
    "orders.total": "Total",
    "account.ordersSome": "Your orders and their current status.",
    "account.viewOrders": "View my orders",
    "status.pending": "Pending",
    "status.paid": "Paid",
    "status.shipped": "Shipped",
    "status.delivered": "Delivered",
    "status.cancelled": "Cancelled",
  },
  fr: {
    "nav.collection": "Collection",
    "nav.theCollection": "La Collection",
    "nav.viewAll": "Tout voir",
    "nav.allPieces": "Toutes les pi\u00e8ces",
    "nav.acquisitions": "Acquisitions",
    "nav.about": "\u00C0 propos",
    "nav.contact": "Contact",
    "nav.menu": "Menu",
    "nav.language": "Langue",
    "nav.currency": "Devise",
    "hero.title": "D\u00e9couvrez des objets remarquables",
    "hero.subtitle": "Livres, Art, Montres, Musique & Plus",
    "hero.cta": "Voir la collection",
    "hero.viewPiece": "Voir la pièce",
    "feat.eyebrow": "S\u00e9lection",
    "feat.title": "Pi\u00e8ces d'exception",
    "auction.viewLots": "Voir les lots",
    "story.quote":
      "\u00AB Chaque objet de notre collection porte une histoire qui m\u00e9rite d'\u00eatre poursuivie. \u00BB",
    "story.body":
      "Balzac Antiques d\u00e9niche, authentifie et pr\u00e9sente livres rares, belles montres et objets remarquables, pour les collectionneurs attach\u00e9s au savoir-faire et \u00e0 la provenance autant qu'\u00e0 la pi\u00e8ce elle-m\u00eame.",
    "badge.ship1": "Livraison",
    "badge.ship2": "internationale",
    "badge.auth1": "Authenticit\u00e9",
    "badge.auth2": "garantie",
    "badge.advice1": "Conseil expert",
    "badge.advice2": "Service personnalis\u00e9",
    "footer.blurb":
      "Livres rares, belles montres et objets remarquables, s\u00e9lectionn\u00e9s et authentifi\u00e9s \u00e0 Bangkok pour les collectionneurs du monde entier.",
    "footer.collections": "Collections",
    "footer.balzac": "Balzac",
    "footer.aboutUs": "\u00C0 propos",
    "footer.stayInformed": "Restez inform\u00e9",
    "footer.brandBlurb": "Livres rares, montres de belle facture et objets remarquables, sourc\u00e9s et authentifi\u00e9s \u00e0 Bangkok pour les collectionneurs du monde entier.",
    "cat.books": "Livres",
    "cat.art": "Art",
    "cat.watches": "Montres",
    "cat.music": "Musique",
    "cat.furniture": "Mobilier",
    "cat.fashion": "Mode",
    "cat.properties": "Propri\u00e9t\u00e9s",
    "cat.curiosities": "Curiosit\u00e9s",
    "footer.newsBlurb": "Nouvelles acquisitions et ventes \u00e0 venir, occasionnellement.",
    "footer.email": "Adresse e-mail",
    "footer.join": "S'inscrire",
    "footer.newsThanks": "Merci. Vous êtes inscrit.",
    "footer.newsErr": "Veuillez saisir une adresse e-mail valide.",
    "footer.rights": "Tous droits r\u00e9serv\u00e9s.",
    "legal.terms": "Conditions de vente",
    "legal.privacy": "Confidentialité",
    "legal.shipping": "Livraison",
    "legal.returns": "Retours",
    "about.eyebrow": "\u00C0 propos",
    "about.title": "Objets rares. Histoires intemporelles.",
    "about.lead":
      "Balzac Antiques est une destination soign\u00e9e pour objets rares et intemporels.",
    "about.p1":
      "Notre collection comprend livres rares, vinyles vintage, belles montres, \u0153uvres d'art, pi\u00e8ces de design iconiques, objets d\u00e9coratifs et pi\u00e8ces de collection soigneusement choisies. Chaque objet est retenu pour son authenticit\u00e9, sa qualit\u00e9, sa raret\u00e9 et son attrait durable.",
    "about.p2":
      "Nous croyons que les beaux objets portent une histoire, un savoir-faire et un caract\u00e8re. Que vous soyez collectionneur averti ou simplement en qu\u00eate d'une pi\u00e8ce unique, nous nous attachons \u00e0 proposer des objets soigneusement sourc\u00e9s, \u00e0 appr\u00e9cier pour les g\u00e9n\u00e9rations \u00e0 venir.",
    "about.p3":
      "S'approvisionnant en Europe et en Asie, Balzac Antiques repose sur une passion pour la qualit\u00e9, l'authenticit\u00e9 et une curation r\u00e9fl\u00e9chie.",
    "about.tagline": "Balzac Antiques \u00b7 Objets rares. Histoires intemporelles.",
    "about.browse": "Parcourir la collection",
    "about.contact": "Nous contacter",
    "contact.eyebrow": "Contact",
    "contact.title": "Nous contacter",
    "contact.lead":
      "Nous serions ravis de r\u00e9pondre \u00e0 vos questions concernant notre collection, vos recherches ou des acquisitions priv\u00e9es.",
    "contact.email": "E-mail",
    "contact.ch": "Suisse",
    "contact.asia": "Asie",
    "contact.forward": "Au plaisir de vous lire.",
    "contact.formTitle": "Envoyer un message",
    "contact.name": "Votre nom",
    "contact.msg": "Message",
    "contact.send": "Envoyer le message",
    "contact.sent": "Merci. Votre message a bien été reçu et la galerie vous répondra par e-mail.",
    "contact.err": "Veuillez vérifier vos informations et réessayer.",
    "sell.eyebrow": "Vendez avec nous",
    "sell.title": "Acquisitions",
    "sell.lead":
      "Balzac Antiques recherche activement livres rares, \u0153uvres d'art, montres vintage, vinyles, design iconique, arts d\u00e9coratifs et pi\u00e8ces de collection de qualit\u00e9.",
    "sell.i1": "Livres rares",
    "sell.i2": "\u0152uvres d'art",
    "sell.i3": "Montres vintage",
    "sell.i4": "Vinyles",
    "sell.i5": "Design iconique",
    "sell.i6": "Arts d\u00e9coratifs",
    "sell.i7": "Pi\u00e8ces de collection",
    "sell.p1":
      "Si vous souhaitez proposer un objet ou une collection enti\u00e8re, envoyez-nous des photographies et une br\u00e8ve description.",
    "sell.p2": "Toutes les demandes sont trait\u00e9es en toute confidentialit\u00e9.",
    "sell.cta": "Proposer un objet",
    "sell.orWrite": "Ou \u00e9crivez-nous directement \u00e0",
    "sell.formTitle": "Parlez-nous de votre pièce",
    "sell.details": "À propos de la pièce",
    "sell.detailsHint": "Objet ou collection, période approximative, état, provenance si connue, et prix souhaité le cas échéant.",
    "sell.send": "Envoyer les détails",
    "sell.sent": "Merci. Votre envoi a bien été reçu et la galerie vous contactera par e-mail.",
    "sell.photos": "Photographies (facultatif)",
    "sell.photosHint": "Jusqu'à 3 images, JPG ou PNG. Des photographies nettes à la lumière du jour nous aident à évaluer une pièce.",
    "sell.confidenceNote": "Toutes les demandes sont traitées en toute confidentialité.",
    "coll.eyebrow": "La Collection",
    "coll.title": "Toutes les pi\u00e8ces",
    "coll.lead":
      "Chaque pi\u00e8ce est choisie pour son authenticit\u00e9, sa qualit\u00e9, sa raret\u00e9 et son attrait durable.",
    "coll.all": "Tout",
    "coll.emptyTitle": "Aucune pi\u00e8ce actuellement dans {c}.",
    "coll.emptyBody":
      "De nouvelles acquisitions arrivent r\u00e9guli\u00e8rement. Si vous cherchez une pi\u00e8ce particuli\u00e8re, nous nous ferons un plaisir de la trouver pour vous.",
    "coll.enquiry": "Faire une demande",
    "coll.viewAllPieces": "Voir toutes les pi\u00e8ces",
    "prod.backTo": "Retour \u00e0",
    "prod.condition": "État",
    "prod.dimensions": "Dimensions",
    "prod.enquire": "Se renseigner sur cette pi\u00e8ce",
    "prod.shipping": "Livraison internationale disponible",
    "prod.sold": "Vendu",
    "prod.soldNote": "Cette pièce a trouvé un nouveau foyer.",
    "prod.reserved": "Réservé",
    "prod.reservedNote": "Cette pièce est actuellement en attente.",
    "drawer.tagline": "Objets rares. Histoires intemporelles.",
    "nav.account": "Mon compte",
    "auth.loginTitle": "Connexion",
    "auth.loginSubtitle": "Accédez à votre compte et à vos commandes.",
    "auth.loginError": "E-mail ou mot de passe incorrect. Veuillez réessayer.",
    "auth.email": "E-mail",
    "auth.password": "Mot de passe",
    "auth.signInCta": "Se connecter",
    "auth.noAccount": "Nouveau chez Balzac Antiques ?",
    "auth.createOne": "Créer un compte",
    "auth.registerTitle": "Créer un compte",
    "auth.registerSubtitle": "Enregistrez vos informations et suivez vos acquisitions.",
    "auth.name": "Nom complet",
    "auth.passwordHint": "Au moins 8 caractères.",
    "auth.createCta": "Créer mon compte",
    "auth.haveAccount": "Vous avez déjà un compte ?",
    "auth.signInLink": "Se connecter",
    "auth.errName": "Veuillez saisir votre nom.",
    "auth.errEmail": "Veuillez saisir une adresse e-mail valide.",
    "auth.errPassword": "Le mot de passe doit contenir au moins 8 caractères.",
    "auth.errTaken": "Un compte existe déjà avec cet e-mail.",
    "account.welcome": "Bienvenue",
    "account.signOut": "Se déconnecter",
    "account.ordersTitle": "Commandes",
    "account.ordersEmpty": "Vous n'avez pas encore de commandes.",
    "account.ordersNote": "Vos acquisitions apparaîtront ici.",
    "account.browse": "Découvrir la collection",
    "account.addressesTitle": "Adresses enregistrées",
    "account.addressesEmpty": "Aucune adresse enregistrée.",
    "account.addressesNote": "Vous pourrez en ajouter lors du paiement.",
    "addr.title": "Adresses",
    "addr.backToAccount": "Retour au compte",
    "addr.backToAddresses": "Retour aux adresses",
    "addr.add": "Ajouter une adresse",
    "addr.manage": "Gérer les adresses",
    "addr.onFile": "Vos adresses de livraison enregistrées.",
    "addr.none": "Vous n'avez aucune adresse enregistrée.",
    "addr.line1": "Adresse",
    "addr.line2": "Appartement, étage, etc. (facultatif)",
    "addr.city": "Ville",
    "addr.postcode": "Code postal",
    "addr.country": "Pays",
    "addr.countryPlaceholder": "Choisir un pays",
    "addr.default": "Par défaut",
    "addr.setDefault": "Définir par défaut",
    "addr.makeDefault": "Utiliser comme adresse par défaut",
    "addr.save": "Enregistrer l'adresse",
    "addr.saveChanges": "Enregistrer les modifications",
    "addr.edit": "Modifier",
    "addr.delete": "Supprimer",
    "addr.err": "Veuillez remplir tous les champs requis.",
    "addr.saved": "Adresse enregistrée.",
    "addr.deleted": "Adresse supprimée.",
    "addr.editTitle": "Modifier l'adresse",
    "nav.cart": "Panier",
    "cart.title": "Votre panier",
    "cart.empty": "Votre panier est vide.",
    "cart.browse": "Découvrir la collection",
    "cart.add": "Ajouter au panier",
    "cart.inCart": "Dans votre panier",
    "cart.view": "Voir le panier",
    "cart.remove": "Retirer",
    "cart.subtotal": "Sous-total",
    "cart.shippingNote": "Frais de livraison calculés au paiement.",
    "cart.checkout": "Passer au paiement",
    "cart.continue": "Continuer mes achats",
    "cart.close": "Fermer",
    "checkout.title": "Paiement",
    "checkout.yourDetails": "Vos coordonnées",
    "checkout.phone": "Téléphone (facultatif)",
    "checkout.shippingAddress": "Adresse de livraison",
    "checkout.useDifferent": "Utiliser une autre adresse",
    "checkout.passwordTitle": "Créer un mot de passe (facultatif)",
    "checkout.passwordHint": "Facultatif. Vous permet de vous connecter plus tard pour suivre votre commande.",
    "checkout.summary": "Votre commande",
    "checkout.reserveNote": "En passant cette commande, la pièce est réservée pour vous. La galerie vous contactera par e-mail pour organiser le paiement et la livraison.",
    "checkout.placeOrder": "Passer la commande",
    "checkout.errUnavailable": "Nous sommes désolés, cette pièce vient d'être acquise par un autre collectionneur :",
    "checkout.removeAndContinue": "La retirer et continuer",
    "checkout.errEmail": "Un compte existe déjà avec cet e-mail.",
    "checkout.payNote": "En passant cette commande, la pi\u00e8ce est r\u00e9serv\u00e9e pour vous et vous \u00eates redirig\u00e9 vers notre page de paiement s\u00e9curis\u00e9e.",
    "checkout.continuePayment": "Passer au paiement",
    "pay.testMode": "Mode test. Aucun paiement r\u00e9el n'est pr\u00e9lev\u00e9. Utilisez le num\u00e9ro de carte 4242 4242 4242 4242, une date d'expiration future, n'importe quel CVC et n'importe quel code postal.",
    "conf.paidEyebrow": "Paiement re\u00e7u",
    "conf.paidTitle": "Merci, votre paiement a bien \u00e9t\u00e9 re\u00e7u",
    "conf.paidNext": "La pi\u00e8ce est \u00e0 vous. La galerie vous contactera par e-mail pour organiser la livraison.",
    "conf.awaitTitle": "Votre commande est r\u00e9serv\u00e9e",
    "conf.awaitNext": "La pi\u00e8ce est r\u00e9serv\u00e9e pour vous et votre commande ne sera finalis\u00e9e qu'apr\u00e8s le paiement. Vous pouvez payer en toute s\u00e9curit\u00e9 ci-dessous.",
    "conf.payNow": "Payer maintenant",
    "conf.payError": "La page de paiement n'a pas pu s'ouvrir pour le moment. Merci de r\u00e9essayer, ou de contacter la galerie qui vous enverra un lien de paiement.",
    "conf.eyebrow": "Merci",
    "conf.title": "Votre commande a bien été reçue",
    "conf.next": "Votre pièce est maintenant réservée pour vous. La galerie vous contactera par e-mail très prochainement pour organiser le paiement et la livraison.",
    "conf.number": "Référence de commande :",
    "conf.items": "Pièces",
    "conf.shipTo": "Adresse de livraison",
    "conf.continue": "Continuer à découvrir la collection",
    "orders.title": "Mes commandes",
    "orders.total": "Total",
    "account.ordersSome": "Vos commandes et leur statut.",
    "account.viewOrders": "Voir mes commandes",
    "status.pending": "En attente",
    "status.paid": "Payée",
    "status.shipped": "Expédiée",
    "status.delivered": "Livrée",
    "status.cancelled": "Annulée",
  },
};
